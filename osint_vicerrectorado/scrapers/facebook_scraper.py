...
Recolecta posts públicos de páginas de Facebook sin necesidad de API.
Utiliza Playwright con técnicas anti-detección para evitar bloqueos.
...
class FacebookScraper(BaseScraper):
    ...
    async def collect_data(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        2. Maneja popups/modales
        3. Hace scroll para cargar más posts
        4. Extrae información de cada post
        ...
        while posts_found < limit and scroll_count < max_scrolls:
            # Obtener HTML actual
            html_content = await self.get_page_html()
            
            # Parsear y extraer posts
            new_posts = await self._parse_posts_from_html(html_content, limit - posts_found)
            ...
            self.logger.info(f"Posts recolectados: {posts_found}/{limit}")
